...
def sn_non_local_block_sim(x, update_collection, name, init=tf.contrib.layers.xavier_initializer()):
    with tf.variable_scope(name):
        ...
        attn = tf.nn.softmax(attn)  # [batch_size, location_num, downsampled_num]
        print('reduce_sum(attn):', tf.reduce_sum(attn, axis=-1))
        ...
        attn_g = tf.matmul(attn, g)  # [batch_size, location_num, num_channels // 2]
