...
flags.DEFINE_integer('number_classes', 1, 'The number of classes in the dataset')
...
# def _get_d_real_loss_cross_entropy(discriminator_on_data_logits):
#     shape = discriminator_on_data_logits.shape
#     loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=discriminator_on_data_logits, labels=tf.ones(shape))
#     return tf.reduce_mean(loss)
...
def _get_d_fake_loss(discriminator_on_generator_logits):
    """
    Hinge Loss
    :param discriminator_on_generator_logits: logits
    :return: loss
    """
    return tf.reduce_mean(tf.nn.relu(1. + discriminator_on_generator_logits))

# def _get_d_fake_loss_cross_entropy(discriminator_on_generator_logits):
#     shape = discriminator_on_generator_logits.shape
#     loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=discriminator_on_generator_logits, labels=tf.zeros(shape))
#     return tf.reduce_mean(loss)
...
# def _get_g_loss_cross_entropy(discriminator_on_generator_logits):
#     shape = discriminator_on_generator_logits.shape
#     loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=discriminator_on_generator_logits, labels=tf.ones(shape))
#     return tf.reduce_mean(loss)
...
class SNGAN(object):
    ...
    def build_model_single_gpu(self, gpu_idx=0, batch_size=1, num_towers=1):
        ...
        generator_fn = generator_module.generator_without_condition

        generator, _ = generator_fn(
            self.zs[gpu_idx],
            self.gf_dim,
        )
        ...
        discriminator_fn = disc.discriminator_without_condition

        discriminator_on_data_logits, _ = discriminator_fn(images, self.df_dim, update_collection=None)
        discriminator_on_generator_logits, _ = discriminator_fn(generator, self.df_dim, update_collection="NO_OPS")
